...
toggle_ipv6() {
    ...
    # 检测 sysctl 中是否禁用了 ipv6
    # 0 代表未禁用（即开启），1 代表禁用了（即关闭）
    local disable_all=$(sysctl -n net.ipv6.conf.all.disable_ipv6 2>/dev/null)
    local disable_default=$(sysctl -n net.ipv6.conf.default.disable_ipv6 2>/dev/null)
    
    # 兜底检测：如果没有 sysctl 权限或命令，尝试看网卡是否有 inet6 地址
